...
description: 在需要与当前工作区隔离开始功能工作，或在执行实现计划之前使用 - 创建隔离的 git 工作区，具有智能目录选择和安全验证
---
...
Git 工作区创建共享同一仓库的隔离工作区，允许同时在多个分支上工作而无需切换。
...
- **修复：** 在创建项目本地工作区之前始终使用 `git check-ignore`
...
工作区就绪于 /Users/jesse/myproject/.worktrees/auth
测试通过（47 个测试，0 个失败）
准备实现 auth 功能
```
...
**被以下技能调用：**
- **brainstorming**（第4阶段）- 设计批准且需要实现时必需
- 任何需要隔离工作区的技能

**与以下技能配对：**
- **finishing-a-development-branch** - 工作完成后清理必需
- **executing-plans** 或 **subagent-driven-development** - 在此工作区中进行工作
